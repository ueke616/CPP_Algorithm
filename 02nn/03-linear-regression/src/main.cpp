...
namespace Application{
	...
	namespace statistics{
		...
		tuple<Item, Item> compute_mean_std(const vector<Item>& items){
			...
			if (items.empty()) {
				cerr << "items向量为空, 无法计算均值和标准差。" << endl;
				return make_tuple(mean, stdval); // 返回默认值或处理错误
			}
	...
	int run() {
		auto datas = io::load_data("../workspace/shanghai.csv");

		for(auto i = datas.begin(); i != datas.end(); i++) {
			printf("year: %f, price: %f\n", i->year, i->price);
		}
		...
		for(int iter = 0; iter < 1000; ++iter) {
			...
			for(auto& item: datas){
				float predict 	= k_identity * item.year + k_sin * std::sin(item.year) 
					+ k_cos * std::cos(item.year) + b;
				float L 	= 0.5 * std::pow(predict - item.price, 2.0f) 
					+ lambda * (k_identity*k_identity + k_sin*k_sin + k_cos*k_cos + b*b);
